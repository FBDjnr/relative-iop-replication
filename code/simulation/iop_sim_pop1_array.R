...
#
#  Sampling     : Stage-1 SRS WOR  — NC clusters per stratum (NC = SGE_TASK_ID)
#                 NC sweeps {25, 50, ..., 750} via UGE array directive -t 25-750:25
...
set.seed(2025)
# On UGE the scheduler exports NSLOTS (= the -pe smp count); honour it so the
# whole core allocation is used. Fall back to detectCores() for interactive runs.
n_cores <- suppressWarnings(as.integer(Sys.getenv("NSLOTS", unset = NA)))
if (is.na(n_cores) || n_cores < 1L) n_cores <- max(1L, availableCores() - 1L)
...
# ---- ARRAY PARAMETRIZATION: NC *is* the UGE array task id -------------------
#  The grid lives in ONE place: the -t directive of the submit script, e.g.
#      #$ -t 25-750:25      ->  tasks 25, 50, 75, ..., 750
#  SGE_TASK_ID is therefore the value of NC directly, so this script just reads
#  whatever NC the scheduler hands it. Change the grid by editing only -t.
NC <- suppressWarnings(as.integer(Sys.getenv("SGE_TASK_ID", unset = NA)))
if (is.na(NC)) {
  NC <- 50L                                  # default for interactive/non-array runs
  cat("  SGE_TASK_ID not set; defaulting to NC = 50.\n")
}
if (NC < 1L || NC > 750L)                    # 750 = H_s, the clusters available per stratum
  stop(sprintf("NC (= SGE_TASK_ID) = %d is out of range 1..750", NC))

B     <- 5000L
HH    <- 4L       # households per cluster
B_NV  <- 300L     # bootstrap draws for naive SE

cat(sprintf("  Array task NC = %d clusters/stratum\n", NC))
...
# Colour constants for black-and-white printing
COL_JK    <- "darkgray"   # dark gray
COL_NAIVE <- "gray50"     # medium gray
COL_TRUE  <- "black"       # black dashed line
COL_PWR   <- "gray30"      # darker gray
COL_T1    <- "gray70"      # lighter gray
...
fig2 <- ggplot(data.frame(Rhat = Rhat_v), aes(x = Rhat)) +
  ...
  geom_density(colour = COL_TRUE, linewidth = 1.0) +
...
fig4 <- ggplot(pwr_df, aes(x = delta, y = rej_rate)) +
  geom_line(colour = COL_TRUE, linewidth = 1.1) +
  ...
  geom_text(aes(label = sprintf("%.3f", rej_rate)),
            vjust = -0.9, size = 3.2) +
  scale_colour_manual(
    values = c("Type I Error" = COL_TRUE, "Power" = COL_JK),
    name   = NULL
  ) +
  scale_shape_manual(
...
ggsave(paste0("fig1_qq_normality_nc", NC, ".pdf"),   fig1, width = 10, height = 5.5)
ggsave(paste0("fig2_sampling_dist_nc", NC, ".pdf"),  fig2, width =  9, height =  5)
ggsave(paste0("fig3_coverage_nc", NC, ".pdf"),       fig3, width =  7, height =  5)
ggsave(paste0("fig4_power_nc", NC, ".pdf"),          fig4, width =  9, height =  5.5)

# ---- Combined 2×2 panel  (for paper) ----------------------------------------
pdf(paste0("iop_sim_pop1_all_figures_nc", NC, ".pdf"),
           width = 16, height = 11)
...
write.csv(tab1,   paste0("tab1_consistency_nc", NC, ".csv"),    row.names = FALSE)
write.csv(tab2,   paste0("tab2_coverage_nc", NC, ".csv"),       row.names = FALSE)
write.csv(tab3,   paste0("tab3_type1_power_nc", NC, ".csv"),    row.names = FALSE)
write.csv(pwr_df, paste0("tab3_power_detail_nc", NC, ".csv"),   row.names = FALSE)
...
cat(sep, "\n")
cat("  Output files\n")
cat("  Figures  : ", paste0("fig1_qq_normality_nc", NC, ".pdf"), "  ", paste0("fig2_sampling_dist_nc", NC, ".pdf"), "\n")
cat("             ", paste0("fig3_coverage_nc", NC, ".pdf"), "  ", paste0("fig4_power_nc", NC, ".pdf"), "\n")
cat("             ", paste0("iop_sim_pop1_all_figures_nc", NC, ".pdf"), "\n")
cat("  Tables   : ", paste0("tab1_consistency_nc", NC, ".csv"), "  ", paste0("tab2_coverage_nc", NC, ".csv"), "\n")
cat("             ", paste0("tab3_type1_power_nc", NC, ".csv"), "  ", paste0("tab3_power_detail_nc", NC, ".csv"), "\n")
cat(sep, "\n")
cat("  Simulation complete.\n")


# Save the R workspace
save.image(paste0("iop_sim_pop1_workspace_nc", NC, ".RData"))